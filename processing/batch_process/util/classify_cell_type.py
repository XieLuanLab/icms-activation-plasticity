...
def get_trough_to_peak_ms(template):
    sampling_rate_khz = 30000
    trough_x = np.argmin(template[25:35]) + 25
    trough_y = template[trough_x]

    peak_index_relative = np.argmax(template[trough_x:60])
    peak_x = trough_x + peak_index_relative
    peak_y = template[peak_x]
    trough_to_peak_ms = 1000 * (peak_x - trough_x) / (sampling_rate_khz)

    return trough_to_peak_ms, trough_x, peak_x, trough_y, peak_y
...
def classify_units_into_cell_types(analyzer):
    ...
    # templates_dict = {}
    templates_dict = template_util.get_unit_primary_ch_templates(analyzer)
    ...
    trough_to_peak_values = {
        unit_id: get_trough_to_peak_ms(template)[0]  # just the first value
        for unit_id, template in templates_dict.items()
    }

    spike_count_values = analyzer.sorting.count_num_spikes_per_unit()
...
def plot_primary_wvf_with_acg_with_fit(result, unit_id, template):
    x = np.arange(1, 101) / 2

    fig, axes = plt.subplots(1, 2, figsize=(12, 6))

    x = np.arange(template.shape[0]) / 30
    # min_values_within_range = np.min(template[20:40, :], axis=0)
    # global_min_index = np.argmin(min_values_within_range)
    axes[0].plot(x, template,
...
# %% Evaluate cell type classification
