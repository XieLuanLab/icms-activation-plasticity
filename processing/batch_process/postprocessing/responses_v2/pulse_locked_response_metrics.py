...
def get_spike_prob_distribution(raster_array, timing_params, subsample_fraction=1):
    ...
    if subsample_fraction < 1.0:
        subsampled_trials = np.random.choice(
            total_trials, num_subsampled_trials, replace=False)
    ...
    prob_spike[(all_bin_centers < post_stim_blank_ms) | (
        all_bin_centers > (pulse_win_ms - pre_stim_blank_ms))] = 0
    ...
    bin_centers = np.insert(all_bin_centers, insert_index, [
                            post_stim_blank_ms, post_stim_blank_ms])
    prob_spike = np.insert(prob_spike, insert_index, [
                           0, prob_spike[insert_index]])
...
def get_bootstrapped_metrics(raster_array, timing_params, subsample_fraction, num_iterations=100, return_summary=True):
...
def calculate_pulse_locked_index(bin_centers, spike_prob_distribution, timing_params):
    ...
    valid_indices = (bin_centers >= post_stim_blank_ms) & (
        bin_centers <= (pulse_win_ms - pre_stim_blank_ms))
    ...
    if max_index == 0:
        mean_top3 = np.mean(
            valid_spike_prob_distribution[max_index: max_index + 3])
    elif max_index == len(valid_spike_prob_distribution) - 1:
        mean_top3 = np.mean(
            valid_spike_prob_distribution[max_index - 2: max_index + 1])
    else:
        mean_top3 = np.mean(
            valid_spike_prob_distribution[max_index - 1: max_index + 2])
...
def get_null_distribution(
    ...
    for _ in range(num_iterations):
        ...
        bins = np.arange(
            0, timing_params["pulse_win_ms"] +
            timing_params["bin_width_ms"], timing_params["bin_width_ms"]
        )
        ...
        phase_lock_index = calculate_pulse_locked_index(
            bin_centers, prob_spike, timing_params)
