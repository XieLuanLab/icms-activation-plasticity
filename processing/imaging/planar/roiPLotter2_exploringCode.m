...
c1 = multcompare(stats);
         ...
         fprintf('Distance analysis skipped: %s\n', ME.message);
     end

%% ========== SAVE INTERMEDIATE AND FINAL RESULTS ==========
% Save key variables for reproducibility without re-running full pipeline
savePath = fullfile(fileparts(mfilename('fullpath')), 'fig3_final_results.mat');
fprintf('\n=== Saving final results to: %s ===\n', savePath);
save(savePath, ...
    ... % --- ROI tracking ---
    'allROI2Comp', 'allDays', 'roiSets', 'roiSetsPx', ...
    'roiEndSets', 'roiEndSetsPx', ...
    ... % --- Longitudinal metrics (per-animal, per-session) ---
    'allDFF', 'allOnset', 'allDur', 'allSpikeNum', 'allSpikeOnset', 'allVol', ...
    'allSubset', 'allTrendsRoiCh', ...
    ... % --- Merged (weekly-binned, all animals) ---
    'mergedDFF', 'mergedTrendsOnset', 'mergedTrendsDur', ...
    'mergedTrendsSpikes', 'mergedTrendsSpikesItime', 'mergedTrendsVol', ...
    'mergedChanData', 'mergedROI2Comp', 'subsetMerged', ...
    ... % --- Current filter ---
    'invalidCurr', ...
    ... % --- End-recruited neurons ---
    'mergedEndDFF', 'mergedEndTrendsOnset', 'mergedEndTrendsDur', ...
    'mergedEndTrendsSpikes', 'mergedEndTrendsSpikesItime', 'mergedEndTrendsVol', ...
    'mergedEndChanData', ...
    ... % --- Raster data ---
    'binROIRaster', 'heatRaster', 'roiRastSum', 'startInds', ...
    ... % --- Metadata ---
    'weekTimepoints', 'fMaxAll', 'bMaxAll', ...
    '-v7.3');
fprintf('Saved successfully. Size: %.1f MB\n', dir(savePath).bytes / 1e6);