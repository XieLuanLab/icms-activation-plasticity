...
% --- AUTO PNG/SVG EXPORT SCRIPT ---
% We identically sweep the verified active masks specifically for this animal

outFld = fullfile(pwd, 'figures', 'Fig3A', 'Weekly_AllActive_ROIs');
if ~exist(outFld, 'dir'), mkdir(outFld); end

fprintf('\nAutomatically Exporting ALL Active ROIs for %s...\n', icms);

for f = 1:numel(files2Load)
    curAllRasterH = validActive{f};
    curDataRaw    = AllData{f};
    curRegionsRaw = curDataRaw.regions;
    
    for ch = 1:numel(chans)
        if chans(ch) ~= target_chan, continue; end
        
        for cu = 1:numel(allCurrs)  
            if allCurrs(cu) ~= target_current, continue; end
            
            if size(curAllRasterH, 1) < ch || size(curAllRasterH, 2) < cu, continue; end
            
            curRaster = curAllRasterH(ch, cu, :);
            curRInds = find(curRaster);
            if isempty(curRInds), continue; end
            
            zProjectedBWCurrent = false(512, 512);
            ...
            for r = 1:numel(curRInds)
                ...
                SE = strel('disk', 1);
                localBW = localBW > 0;
                localBW = imerode(localBW, SE);
                zProjectedBWCurrent(localBW == 1) = 1;
            end
            
            if any(zProjectedBWCurrent, 'all')
                % 1. Create raster RGB PNG (as backup/quick preview)
                rgbImg = ones(512, 512, 3, 'uint8') * 255;
                light_blue_px = [76, 150, 220]; 
                roiIdx = zProjectedBWCurrent == 1;
                R = rgbImg(:,:,1); R(roiIdx) = light_blue_px(1); rgbImg(:,:,1) = R;
                G = rgbImg(:,:,2); G(roiIdx) = light_blue_px(2); rgbImg(:,:,2) = G;
                B = rgbImg(:,:,3); B(roiIdx) = light_blue_px(3); rgbImg(:,:,3) = B;
                
                baseName = sprintf('%s_AllActiveROI_Session%d_Chan%d_Curr%duA', icms, f, chans(ch), allCurrs(cu)); 
                imwrite(rgbImg, fullfile(outFld, [baseName, '.png']));
                
                % 2. Generate mathematically perfect Vector SVGs 
                light_blue_vec = light_blue_px / 255; 
                curFig = figure('Visible', 'off', 'Color', 'w', 'Position', [100, 100, 512, 512]); 
                hold on; axis ij; axis equal; xlim([1 512]); ylim([1 512]); axis off;
                
                boundaries = bwboundaries(zProjectedBWCurrent);
                for k = 1:numel(boundaries)
                    b = boundaries{k};
                    patch(b(:,2), b(:,1), light_blue_vec, 'EdgeColor', 'none'); 
                end
...
fprintf('Success %s! Exported beautifully to: %s\\n', icms, outFld);
