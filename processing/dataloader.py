...
class DataLoader:
    ...
    def remove_high_impedance_ch(self):
        ...
        impedance_analyzer.get_intan_impedances(
            animal_id=self.animalID, server_mount_drive=self.server_mount_drive)
    ...
    def get_base_server_folder(self):
        ...
        self.base_server_folder = os.path.join(
            self.server_mount_drive, icms_id, "Keys", date, subfolder)
    ...
    def get_key_files(self):
        ...
        key_paths = [os.path.join(self.base_server_folder, f.replace(
            "_behavior", "_key")) for f in self.mat_files]
        (
            self.all_currents,
            self.all_depths,
        ...
        self.all_modified_stim_start_stop = []
    ...
    def add_catch_trial_fake_stim_times(self, trial_array, num_pulses=70, interpulse_interval_s=0.01):
        """
        For valid comparison with stim trials, compute times of "fake" stim pulses.
        """
        delay_s = self.calculate_average_stim_onset_delay(trial_array)

        for i, trial in enumerate(trial_array):
            if (trial['Current'] > 0) or (len(trial['StimStartStop']) == 0):
                continue
            first_pulse_s = trial['StimStartStop'][0] + delay_s
            timestamps_s = np.arange(
                first_pulse_s, first_pulse_s + num_pulses * interpulse_interval_s,
                interpulse_interval_s
            )
            trial_array[i]['StimTimestamps'] = timestamps_s

        return trial_array

    def calculate_average_stim_onset_delay(self, trial_array):
        """
        Calculates average time from trial start to first stimulation pulse.
        This is used to determine imaginary first pulse of catch trial which has no actual stim
        but necessary for valid comparison with stim trials.
        """
        trial_starts_s = [trial['StimStartStop'][0]
                          if len(trial['StimTimestamps']) > 0 else np.nan
                          for trial in trial_array]
        first_pulse_times_s = [arr['StimTimestamps'][0] if len(
            arr['StimTimestamps']) > 0 else np.nan for arr in trial_array]
        delays_s = np.array(first_pulse_times_s) - np.array(trial_starts_s)
        return np.nanmedian(delays_s)
    ...
    def _extract_key_data_from_mat(self, mat_data, index):
        ...
        trial_array = self.add_catch_trial_fake_stim_times(trial_array)
        currents = [trial["Current"] for trial in trial_array]
        depths = [trial["Depth"] for trial in trial_array]

        stim_timestamps = [
            (np.array(trial["StimTimestamps"]) * self.fs +
             self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]

        stim_start_stop = [
            (np.array(trial["StimStartStop"]) * self.fs +
             self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        img_start_stop = [
            (np.array(trial["ImageStartStop"]) * self.fs +
             self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        frame_timestamps = [
            (np.array(trial["FrameTimestamps"]) * self.fs +
             self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        ...
        if last_trial > -1:
            ...
            self.modified_stim_start_stop = self.fill_with_nans(
                last_trial, stim_start_stop)
            ...
            self.all_modified_stim_start_stop.extend(
                self.modified_stim_start_stop)
    ...
    def append_recordings(self):
        ...
        durations = [rec_i.get_num_frames(
        ) / self.fs for rec_i in self.recordings_list]
    ...
    def get_concatenated_recording(self):
        ...
        if hasattr(recording, 'select_channels'):
            self.recording = recording.select_channels(self.channel_ids[0:32])
        else:
            self.recording = recording.channel_slice(self.channel_ids[0:32])

    def check_trial_df(self, trial_df):
        for row, trial in trial_df.iterrows():
            trial_start = trial['trial_start']
            if pd.notna(trial_start):
                assert trial['trial_end'] >= trial_start
                stim_ts = trial['stim_timestamps']
                for stim_t in stim_ts:
                    assert stim_t > trial_start

                if trial['current'] == 0:
                    stim_ts = trial['stim_timestamps']
                    assert stim_ts[0] > trial_start
    ...
    def get_trial_dataframe(self):
        ...
        if len(self.all_stim_timestamps) < num_trials:
            ...
            df_stim_start_stop = self.all_modified_stim_start_stop
        else:
            df_stim_ts = self.all_stim_timestamps
            df_frame_ts = self.all_frame_timestamps
            df_img_start_stop = self.all_image_start_stop
            df_stim_start_stop = self.all_stim_start_stop

        def safe_cast_list(sublist):
            if isinstance(sublist, list) and sublist != [np.nan]:
                return [int(t) for t in sublist]
            return np.nan
        ...
        trial_df = pd.DataFrame(
            {
                ...
                "trial_start": [
                    int(sublist[0]) if isinstance(sublist, list) and sublist != [
                        np.nan] else np.nan
                    for sublist in df_stim_start_stop
                ],
                "trial_end": [
                    int(sublist[1]) if isinstance(sublist, list) and sublist != [
                        np.nan] else np.nan
                    for sublist in df_stim_start_stop
                ],

                "stim_timestamps": [safe_cast_list(sublist) for sublist in df_stim_ts],
                "frame_timestamps": [safe_cast_list(sublist) for sublist in df_frame_ts],
        ...
        for block in range(num_blocks):
            ...
            for channel in unique_channels:
                ...
                psignifit_data = np.column_stack(
                    [grouped_data["current"], grouped_data["responses"],
...
if __name__ == "__main__":
    ...
    # test with truncated session
    data_folder = "C:\\data\\ICMS98\\02-Nov-2023"
    # data_folder = "C:\\data\\ICMS93\\Behavior\\26-Sep-2023"
